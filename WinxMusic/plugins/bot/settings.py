    ...
    if callback_query.message.chat.type == ChatType.PRIVATE:
        ...
        try:
            await callback_query.edit_message_text(
                _["start_1"].format(app.me.username),
                reply_markup=InlineKeyboardMarkup(buttons),
            )
...
"""✅<u>Group Settings:</u>
/settings - Get full group settings with inline buttons.

🔗 <u>Options in Settings:</u>

1. You can set the Audio Quality.
2. You can set the Video Quality.
3. Authorized Users : You can change the mode of admin commands to "everyone" or "admins only".
...
/playmode - Get the full playback settings panel with buttons, where you can adjust group playback settings.

<b><u>Playmode Options:</u></b>
1. Seek Mode [Direct or Inline] - Changes the seek mode when using the /playmode command.
2. Admin Commands [Everyone or Admins] - If "everyone", anyone in the group will be able to use admin commands (like /skip , /stop , etc).
3. Playback Type [All or Admins] - If "admins", only group admins will be able to play music in voice chat.
"""